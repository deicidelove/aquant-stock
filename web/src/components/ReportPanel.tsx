  ...
  return (
    <section className="rounded-lg border border-slate-700 p-4">
      <div className="flex items-baseline justify-between">
        ...
        <span className="rounded bg-slate-800 px-2 py-1 text-sm"><span>{d.signal}</span>（{d.total_score} 分 · 风险{d.risk_level}）</span>
      </div>
      <p className="mt-2 text-sm text-slate-300">{d.one_liner}</p>
      ...
      <div className="mt-2 text-xs text-slate-500">风险：{d.risks.join("；")}</div>
    </section>
  );
}
